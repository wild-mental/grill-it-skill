...
5. **해소 즉시 영속화.** 한 토픽이 결정되면 다음 질문으로 넘어가기 전에 (a) 관련 설계문서와 (b) 에이전트 하네스(규칙·스킬·서브에이전트·워크플로/명령·훅 등 결정 성격에 맞는 계층)를 수립·수정하고 (c) Grill Ledger에 RESOLVED로 기록한다. 결정을 메모리에만 두지 않는다.
