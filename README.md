...
**나를 추궁하는 게 아니라, 나와 함께 "문제"를 굽는 Skill입니다. 착수 전에 모호한 결정 토픽을 전부 끄집어내 한 토픽씩 함께 해소하고, 결정형 질문은 가능한 경우 선택형 UI로 띄우며, 해소되는 즉시 설계문서와 에이전트 하네스에 반영합니다. Cursor, Claude Code, Codex 모두 지원합니다.**
...
### 4. 결정형 질문은 선택형 UI로 묻는다

토픽마다 선택지와 권장안을 평문으로만 던지지 않습니다. Agent Client Protocol의 `AskUserQuestion` 같은 클라이언트 질문 UI를 사용할 수 있으면 **권장안을 첫 옵션**에 두고 선택형 UI로 묻습니다. 사용자는 매번 긴 답을 작성하지 않고 클릭 또는 짧은 교정으로 진행할 수 있습니다. 질문 UI가 없는 환경에서만 평문 질문으로 fallback합니다.

### 5. 해소되는 즉시 문서·하네스에 반영한다 (Persistence)
...
| 터미널 평문 질문에 직접 답해야 함 | 가능한 경우 Agent Client Protocol `AskUserQuestion` 선택형 UI로 응답 |
...
| Step 2 — Grill the Topic | 첫 UNRESOLVED 토픽 1개: 선택지 + **권장안+근거** + 선택형 UI 단일 질문(`AskUserQuestion`, 권장안 첫 옵션; UI 없으면 평문 fallback) |
...
| 기본 원칙 | 문제를 주어로 · 범위 한정 · 토픽 선(先)가시화 · 한 번에 하나 · 선택형 질문 UI · 해소 즉시 영속화 · 재개 가능 |
...
- 결정형 질문을 터미널 평문이 아니라 **선택형 UI로 편하게 답하고** 싶은 분
