---
name: grill-it
description: 사용자가 아니라 "문제"를 함께 굽는다. 착수(구현) 전에, 사용자가 지정한 참조 범위와 관심 방향 안에서 미해소 결정 토픽 전체를 먼저 추출해 가시화하고, 의존 순서대로 한 토픽씩 권장안과 함께 해소하며, 해소되는 즉시 설계문서(PRD/SRS/Tasks/Issues)와 에이전트 하네스(rules/AGENTS.md/CLAUDE.md)에 반영하고 재개 가능한 Grill Ledger에 기록한다. Use when the user wants to de-risk a plan/design before building, clear ambiguities up front, mentions "grill-it", "grill the problem", "문제를 굽자", "착수 전 모호함 정리", or wants a scoped, resumable decision-clearing session.
---
...
4. **Resolve T1**: 사용자 "권장안대로" → `docs/SRS.md` 데이터 섹션 갱신 + `.cursor/rules/data-model.mdc`에 "기본 저장소 = Postgres" 반영 + 원장 `RESOLVED 1 / TOTAL 3`. → `[반영 완료]` 후 T2로.
